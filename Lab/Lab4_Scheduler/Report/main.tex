...
\reporttype{\Large Report Lab 4}
\title{\Large A cooperative scheduler}
...
\subsection{Define and initialize the variables and data structures for the scheduler}

\begin{lstlisting}[basicstyle=\small, caption=An implementation of the Define and initialize]
#include <stdlib.h>
...
Array \textbf{TaskList} is used to store the tasks that are added to the scheduler. Each task will have its own delay time, period time, and RunMe flag.

Linked list \textbf{WaitingList} is used to store the IDs of the tasks that are waiting to be executed. Linked list \textbf{ActiveList} is used to store the IDs of the tasks that are ready to be executed.

\subsection{SCH\_Intit}

\begin{lstlisting}[basicstyle=\small, caption=An implementation of the ‘SCH\_Intit’ functions]
void SCH_Intit(void){
	WaitingList = NULL;
	ActiveList = NULL;
	numTask = 0;
...
Function \textbf{SCH\_Init} is used to initialize the variables and data structures for the scheduler. 
\subsection{SCH\_Add\_Task}
\begin{lstlisting}[basicstyle=\small, caption=An implementation of the ‘SCH\_Add\_Task’ functions]
...
\end{lstlisting}
Function \textbf{SCH\_Add\_Task} is used to add a task to the scheduler. It takes three parameters: a pointer to the function to be executed, the delay time before the task is executed, and the period time for periodic tasks. It returns the ID of the added task.

\begin{lstlisting}[basicstyle=\small, caption=An implementation of the ‘insert’ functions]
void insert(Node* i){
	if(WaitingList == NULL){
		WaitingList = i;
...
Create Node and add this Node in the WaitingList in the correct position based on the delay time of the task.

\subsection{SCH\_Update}
\begin{lstlisting}[basicstyle=\small, caption=An implementation of the ‘SCH\_Update’ functions]
void SCH_Update(void){
//	Watchdog_Refresh();
	if(numTask <= 0) return;
...
Function \textbf{SCH\_Update} is called in the timer interrupt. It updates the delay time of first task in the WaitingList. If a task's delay time reaches zero, it is moved to the ActiveList and its RunMe flag is incremented.

\subsection{SCH\_Dispatch\_Tasks}

\begin{lstlisting}[basicstyle=\small, caption=An implementation of the ‘SCH\_Dispatch\_Tasks’ functions]
void SCH_Dispatch_Tasks(void){
...
Fuction \textbf{SCH\_Dispatch\_Tasks} is called in the main loop. It executes all tasks in the ActiveList whose RunMe flag is greater than zero. After executing a task, its RunMe flag is decremented. After run this task is removed from the ActiveList.

\subsection{SCH\_Delete\_Task}

\begin{lstlisting}[basicstyle=\small, caption=An implementation of the ‘SCH\_Delete\_Task’ functions]
...
This function is used to delete a task from the scheduler based on its ID. It removes the task from the WaitingList and resets its parameters in the TaskList.

\subsection{SCH\_Go\_To\_Sleep}
...
Function \textbf{SCH\_Go\_To\_Sleep} puts the MCU into sleep mode to reduce power consumption when there are no tasks to execute and will acivate again when an interrupt occurs.

\subsection{in while(1) loop in main.c}
\begin{lstlisting}[basicstyle=\small, caption=An implementation of the ‘watchdog’ functions]
init_running();
SCH_Intit();
...
Add all task of lab 3 to the scheduler. In the while(1) loop, call \textbf{SCH\_Dispatch\_Tasks} to execute the tasks.

Test code for delete and add task in the while(1) loop.
