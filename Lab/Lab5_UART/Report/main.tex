...
\reporttype{\Large Report Lab 5}
\title{\Large Flow and Error Control in Communication}
...
\begin{figure}[!htp]
    \centering
    \includegraphics[width=5in]{graphics/Proteus_Simulation.PNG}
...
\begin{figure}[!htp]
    \centering
    \includegraphics[width=4in]{graphics/Proteus_Simulation2.PNG}
...
\begin{figure}[!htp]
    \centering
    \includegraphics[width=4.5in]{graphics/stm32_uart_1.PNG}
...
\begin{figure}[!htp]
    \centering
    \includegraphics[width=3.5in]{graphics/stm32_uart_2.PNG}
...
\begin{figure}[!htp]
    \centering
    \includegraphics[width=4.5in]{graphics/stm32_adc_1.PNG}
    \caption{\textit{Enable UART interrupt}}
    \label{3}
\end{figure}
...
\subsubsection{Command parser FSM}
The command parser FSM is designed as follows:

\begin{figure}[!htp]
    \centering
    \includegraphics[width=4.5in]{graphics/f1.png}
    \caption{\textit{FSM for command parser}}
    \label{4}
\end{figure}

The program code is implemented as follows:

\begin{lstlisting}[caption= Program code for command parser FSM]
...
With \text{SendData = 0} is INIT state, \text{SendData = 1} is READ\_SENSOR state, \text{SendData = 2} is SEND\_DATA state and \text{SendData = 3} is WAIT\_ACK state.

\subsubsection{UART communication FSM}

This implementation is not use FSM to extract the command from the buffer. Instead, it checks the last characters in the buffer to find out if there is a valid command. The program skeleton is proposed as follows:

\begin{lstlisting}[caption= Program structure for UART communication FSM]
...
with \textbf{Inst = 1} is instruction \textbf{!RST\#} and \textbf{Inst = 2} is instruction \textbf{!OK\#}.

\subsubsection{While loop and UART}
Finally, the main while loop is implemented as follows:
...
\begin{lstlisting}[caption= Program for UARTRx complete callback]
void HAL_UART_RxCpltCallback(UART_HandleTypeDef * huart){
	if( huart->Instance == USART2 ) {
		buffer[index_buffer++] = temp ;
		if(index_buffer==30) index_buffer = 0;;
